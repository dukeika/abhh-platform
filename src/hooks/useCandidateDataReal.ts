...
export function useCandidateData() {
  ...
  const applyToJob = async (jobId: string, applicationData?: { coverLetter?: string; resumeUrl?: string }) => {
    if (!user) {
      throw new Error("User not authenticated");
    }

    try {
      console.log('🔄 Applying to job with data:', { jobId, applicationData });
      console.log('🔍 User info:', { userId: user.userId, username: user.username });
      
      // Get the user's actual database ID (not Cognito sub)
      const { userService } = await import('@/services/userService');
      const userRecord = await userService.getUserBySub(user.userId || user.username);
      ...
      const newApplication = await applicationService.applyToJob(
        userRecord.id, // Use the User table ID, not the Cognito sub
        jobId,
        applicationData
      );
